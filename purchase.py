...
class Purchase:
    def __init__(self, purchase_page):
        ...
        payFrame = LabelFrame(purchase_page, text='Purchase Info', font=("Poppins SemiBold", 15, 'italic'),
                              bg='#BBD0FF', bd='1.5', width=450, height=640)
        payFrame.place(x=55, y=70)
                ...
                if cash_name.get() == "":
                    ...
                    if count.find('Total') == -1:
                        ...
                        cur.execute(insert, [productName_combo.get(), productBrand_combo.get(), productQty_entry.get(),
                                             purchaseDate_entry.get(), bill_num.get(), cash_name.get(), purchaseStatus_combo.get()])
                        Db.commit()
        ...
        clearEntry = Button(purchase_page, text="Clear Purchase Info ", font=("Poppins SemiBold", 10, 'italic'),
                            bg='white', fg='black', activebackground='#BBD0FF', width=17, border=0, cursor='hand2',
                            justify=CENTER, command=clear_purchaseFrame)
        clearEntry.place(x=320, y=630)
